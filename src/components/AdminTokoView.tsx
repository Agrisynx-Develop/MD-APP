              ...
              <table className="w-full text-xs text-left">
                ...
                <tbody className="divide-y divide-slate-100">
                  {STANDARD_PLANS.map((plan) => {
                    ...
                    const stockAwal = rec
                      ? rec.openingStockKg
                      : carryItems.reduce((sum, i) => sum + i.weightBeforeThawing, 0);
                    const bahanDiolah = planItems.reduce((sum, i) => sum + i.weightBeforeThawing, 0);
                    ...
                    const hasilPotong = planItems.reduce(
                      (sum, i) => sum + (i.weightAfterThawing || i.weightBeforeThawing),
                      0
                    );
                    const totalTersedia = stockAwal + hasilPotong + adjIn - adjOut;
                    const sales = rec ? rec.salesKg : planSegs.reduce((sum, s) => sum + (s.salesKg || 0), 0);
                    const stokSistem = Math.max(0, totalTersedia - sales);
                    const stokReal = rec
                      ? rec.actualClosingStockKg
                      : planSegs.reduce((sum, s) => sum + s.actualWeight, 0);
                    const susutJual = rec ? rec.susutJualKg : Math.max(0, stokSistem - stokReal);
